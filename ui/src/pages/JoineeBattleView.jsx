...
const JoineeBattleView = () => {
    ...
    useEffect(() => {
        ...
        const handleMoveToNext = () => {
            showNotification("🚀 Someone solved it! Advancing to next round!", "warning");
        ...
        socket.on('battle_ended', () => { alert("Battle Ended!"); navigate('/'); });
        socket.on('kicked', () => { alert("You have been kicked."); navigate('/'); });
    ...
    const handleCodeUpdate = (val) => {
        ...
        if (socket && battleData?.questions[activeQIndex]) {
            ...
            socket.emit('save_battle_code', {
                battle_id,
                question_id: battleData.questions[activeQIndex].question_id,
                code: val,
    ...
    // --- RENDER ---
    if (!battleData) return <div className="h-screen flex items-center justify-center bg-[#0D1117] text-white">Connecting to Battlefield...</div>;

    if (battleData.status === 'DRAFT') return (
        <div className="h-screen flex flex-col items-center justify-center bg-[#0D1117] text-white gap-6">
            ...
            <h1 className="text-4xl font-bold tracking-tighter">PREPARE FOR BATTLE</h1>
            ...
            <div className="bg-[#161B22] p-4 rounded-lg border border-gray-800">
                <h3 className="text-sm font-bold text-gray-500 uppercase mb-2">Lobby ({participants.length})</h3>
                <div className="flex gap-2 flex-wrap justify-center max-w-md">
                    {participants.map(p => (
                        <span key={p.id} className="bg-gray-800 px-2 py-1 rounded text-xs text-blue-300 border border-blue-900/30">{p.name}</span>
                    ))}
                </div>
            </div>
        </div>
    ...
    if (activeQIndex >= battleData.questions.length) {
        return (
            <div className="h-screen flex flex-col items-center justify-center bg-[#0D1117] text-white gap-6">
                ...
                <button onClick={() => navigate('/')} className="px-6 py-2 bg-blue-600 rounded">Exit Arena</button>
            </div>
        );
    }
    ...
    return (
        <div className="h-screen bg-[#0D1117] text-gray-300 flex flex-col overflow-hidden font-sans">
            ...
            {/* NOTIFICATION TOAST */}
            {notification && (
                <div className={`fixed top-20 left-1/2 transform -translate-x-1/2 z-50 px-6 py-3 rounded-lg shadow-2xl border flex items-center gap-3 animate-bounce
                    ${notification.type === 'success' ? 'bg-green-600 border-green-400 text-white' : 'bg-yellow-600 border-yellow-400 text-white'}`}>
            ...
            <header className="bg-[#161B22] border-b border-gray-800 p-3 flex justify-between items-center shadow-md z-10">
                <div className="flex items-center gap-4">
                    <div className="flex gap-1">
                        {battleData.questions.map((_, idx) => (
                            <div key={idx} className={`h-2 w-8 rounded-full ${idx === activeQIndex ? 'bg-blue-500 shadow-lg shadow-blue-500/50' : idx < activeQIndex ? 'bg-green-500' : 'bg-gray-700'}`} />
                        ))}
                    </div>
                    <span className="font-bold text-white text-sm">Round {activeQIndex + 1}/{battleData.questions.length}</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">Opponents: {participants.length - 1}</span>
                    <button onClick={() => { if(window.confirm("Surrender?")) navigate('/') }} className="bg-red-900/30 hover:bg-red-900/50 text-red-400 px-3 py-1 rounded text-xs border border-red-800/50">Surrender</button>
                </div>
            </header>
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="w-1/4 bg-[#0D1117] border-r border-gray-800 flex flex-col">
                    <div className="p-6 overflow-y-auto flex-1">
                        ...
                        <div className="prose prose-invert mb-6 text-gray-300 text-sm whitespace-pre-wrap leading-relaxed">{activeQ.description}</div>
                        {activeQ.example && (
                            <div className="mt-4">
                                <span className="text-xs font-bold text-gray-500 uppercase">Example</span>
                                <pre className="bg-[#161B22] border border-gray-700 p-3 rounded text-xs text-gray-300 whitespace-pre-wrap font-mono mt-1">{activeQ.example}</pre>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="h-1/3 border-t border-gray-800 bg-[#161B22] flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                            {sortedParticipants.map((p, idx) => (
                                <div key={p.id} className={`flex justify-between items-center text-xs p-1.5 rounded ${socket.user?.user_id === p.id ? 'bg-blue-900/30 border border-blue-800' : 'text-gray-400'}`}>
                                    <div className="flex items-center gap-2">
                                        <span className="font-mono w-4">{idx+1}.</span>
                                        <span className={socket.user?.user_id === p.id ? 'text-blue-300 font-bold' : ''}>{p.name} {socket.user?.user_id === p.id && '(You)'}</span>
                                    </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                    <CodeEditor 
                        key={activeQ.question_id} // Force remount on question change to clear state if needed
                        value={currentCode}
                        language={currentLang}
