...
async function setupSessionEvents(socket, io) {
    ...
    socket.on('join_session', async ({ session_id }) => {
        ...
        const role = (hostId === user_id) ? 'host' : 'joinee';

        const redisHostKey = hostId ? `session:code:${session_id}:${hostId}` : null;
    ...
    socket.on('host_code_change', async ({ session_id, new_code }) => {
        const codeToSave = new_code !== undefined ? new_code : '';
        const redisKey = `session:code:${session_id}:${user_id}`;
        await redis.set(redisKey, JSON.stringify({ code: codeToSave, language: 'javascript' }));

        socket.to(session_id).emit('host_code_update', codeToSave);
        ...
        const flush = async () => {
            ...
            await produceEvent(Topics.SESSION_TOPIC.name, {
                ...
                payload: {
                    ...
                    query: query,
                    params: [codeToSave, session_id, user_id]
                },
                key: session_id
            });
        };
    ...
    socket.on('joinee_code_change', async ({ session_id, code }) => {
        ...
        socket.to(session_id).emit('joinee_code_update', { joineeId: user_id, code: codeToSave });
        ...
        const flush = async () => {
            ...
            if (existing.length > 0) {
                ...
                query = 'INSERT INTO session_codes(session_id, user_id, code, code_lang) VALUES(?,?,?,?)';
                params = [session_id, user_id, codeToSave, 'javascript'];
            }
            await produceEvent(Topics.SESSION_TOPIC.name, {
                type: Events.DB_QUERY.type,
                payload: {
