...
export const setupBattleEvents = async (io, socket) => {
            ...
            if (role === 'host') {
                ...
                if (cached && Object.keys(cached).length > 0) {
                    savedCode = Object.entries(cached).map(([qId, val]) => {
                        const { code, language } = JSON.parse(val);
                        return { user_id, battle_question_id: parseInt(qId), code, language };
                    });
                } else {
    ...
    socket.on('save_battle_code', async (data) => {
        ...
        await redis.hSet(redisKey, String(battle_question_id), JSON.stringify({ code, language }));
